...
from settings import RIDE_KEY_SCHEMA_PATH, RIDE_VALUE_SCHEMA_PATH, \
    SCHEMA_REGISTRY_URL, BOOTSTRAP_SERVERS, GREEN_KAFKA_TOPIC, FHV_KAFKA_TOPIC
...
if __name__ == "__main__":
    ...
    # p1 = Process(target = avro_consumer.consume_from_kafka(topics=[GREEN_KAFKA_TOPIC]))
    # p2 = Process(target = avro_consumer.consume_from_kafka(topics=[FHV_KAFKA_TOPIC]))
    # p1.start()
    # p2.start()
    # p1.join()
    # p2.join()
    # avro_consumer.consume_from_kafka(topics=[GREEN_KAFKA_TOPIC])
    avro_consumer.consume_from_kafka(topics=[FHV_KAFKA_TOPIC])
